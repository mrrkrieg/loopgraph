...
import { generateAssessment } from "./fixture-provider";
...
export async function simulateLoop(input: {
  ...
  const startedAt = fixture.simulatedAt;
  ...
  const contextSnapshot = await compileContextSnapshot({ spec: input.spec, fixture });
  const agentOutput = generateAssessment({ spec: input.spec, fixture, context: contextSnapshot });
  ...
  if (shouldCreateEscalationCase(policyDecisions) || agentOutput.escalationRequest?.required) {
    ...
    if (escalationCase) {
      ...
      await storage.saveEscalationCase(escalationCase);
    }
  }
  ...
  const trace: LoopRunTrace = {
    ...
    idempotencyKey: idem,
    contextSnapshot,
    inputs: [{ key: "fixture.eventId", value: fixture.eventId, source: "fixture" }],
    agentOutput,
