...
4. Read the returned `learningContext` and retain its `learningContextDigest`. The context joins bounded routing evaluations, human corrections, observed outcomes, and net-value evidence for eligible and subject-related loops. Treat it as advisory evidence only; an unavailable or empty context is not permission to infer performance.
...
9. Submit the bounded decision with `loopgraph_routing_decision_submit`, echoing `learningContextDigest` and including confidence, evidence references, alternatives, and the reason to abstain when applicable. If Loopgraph rejects a stale digest, re-ingest before reasoning again; never retry with the digest omitted.
