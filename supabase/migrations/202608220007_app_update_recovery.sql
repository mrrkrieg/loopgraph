-- Extend the App lifecycle journal to reviewed update materialization. Every
-- update is bound to one actor, content-bound plan, permission approval set,
-- source installation and ownership graph, and exact source or target LoopSpec
-- inventory accepted during recovery.
        ...
        or exists (
          ...
          where jsonb_typeof(item.value) is distinct from 'object'
            or item.value - array[
              ...
              'targetArtifactDigest', 'status', 'desired', 'activation', 'rollout', 'uninstall', 'update', 'rollback', 'actor',
            ...
            or coalesce(item.value->>'action', '') not in ('install', 'uninstall', 'activate', 'pause', 'resume', 'update', 'rollback')
            ...
            or (item.value->>'resultReceiptId' is not null and item.value->>'action' not in ('uninstall', 'update', 'rollback'))
