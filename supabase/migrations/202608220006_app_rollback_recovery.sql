-- Extend the App lifecycle journal to rollback rematerialization. Every rollback
-- is bound to one source installation and ownership graph, one prior revision,
-- and the exact source or target LoopSpec inventory accepted during recovery.
        ...
        or exists (
          ...
          where jsonb_typeof(item.value) is distinct from 'object'
            or item.value - array[
              ...
              'targetArtifactDigest', 'status', 'desired', 'activation', 'rollout', 'uninstall', 'rollback', 'actor',
            ...
            or coalesce(item.value->>'action', '') not in ('install', 'uninstall', 'activate', 'pause', 'resume', 'rollback')
            ...
            or (item.value->>'resultReceiptId' is not null and item.value->>'action' not in ('uninstall', 'rollback'))
