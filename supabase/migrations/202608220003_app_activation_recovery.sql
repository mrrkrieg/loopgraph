-- Extend the metadata-only App lifecycle journal to activation. An activation
-- operation binds one exact approval receipt, artifact, source state, target
-- mode, actor, and owned LoopSpec set so a cross-store interruption can resume
-- without issuing new authority or replaying provider work.
        ...
        or exists (
          ...
          where jsonb_typeof(item.value) is distinct from 'object'
            or item.value - array[
              ...
              'targetArtifactDigest', 'status', 'desired', 'activation', 'actor',
            ...
            or coalesce(item.value->>'action', '') not in ('install', 'uninstall', 'activate')
