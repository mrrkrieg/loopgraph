-- Durable, metadata-only recovery records for cross-store App install/uninstall work.

alter table public.loopgraph_app_installation_registries
  add constraint loopgraph_app_installation_lifecycle_operations_shape
  check (
    registry_payload->'lifecycleOperations' is null
    or (
      jsonb_typeof(registry_payload->'lifecycleOperations') = 'array'
      and jsonb_array_length(registry_payload->'lifecycleOperations') <= 100
    )
  );
        ...
        or exists (
          ...
          where jsonb_typeof(item.value) is distinct from 'object'
            or item.value - array[
              ...
              'targetArtifactDigest', 'status', 'desired', 'actor', 'startedAt',
              'updatedAt', 'completedAt', 'resultReceiptId', 'failureCode'
            ]::text[] <> '{}'::jsonb
            ...
            or coalesce(item.value->>'action', '') not in ('install', 'uninstall')
