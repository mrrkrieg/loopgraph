...
/** Design Studio flat loop spec shape (pre-v1alpha1). */
export const flatLoopSpecSchema = z.object({
  id: z.string(),
  name: z.string().min(1),
...
export type FlatLoopSpec = z.infer<typeof flatLoopSpecSchema>;

export function validateFlatLoopSpec(input: unknown): FlatLoopSpec {
  return flatLoopSpecSchema.parse(input);
}
