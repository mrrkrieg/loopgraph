...
const packRoot = path.resolve(process.cwd(), "packs/official/sales/find-recover-cold-deals");

describe("official Find and Recover Deals Going Cold app", () => {
  it("compiles the complete five-loop deal-risk topology", async () => {
    ...
    expect(compiled.appId).toBe("loopgraph.sales.find-recover-cold-deals");
    expect(compiled.loopSpecs.map((loop) => loop.metadata.id)).toEqual([
      "sales-deal-risk-detection",
      "sales-missing-next-step",
      "sales-stakeholder-analysis",
      "sales-deal-follow-up",
      "sales-pipeline-outcome"
    ]);
  ...
  it("provides both sales-stack recipes, guided setup, and complete conformance coverage", async () => {
    const loaded = await loadLoopPackDirectory(packRoot, {
      loopgraphVersion: "0.2.0",
      hermesVersion: "1.0.0"
    });
    ...
    expect(loaded.manifest.presets.map((preset) => preset.id)).toEqual([
      ...
      "salesforce-outlook-calendar-teams"
    ]);
    expect(connectors).toHaveLength(2);
  ...
  it("passes routing, approval, replay, and upgrade cases without provider writes", async () => {
    const loaded = await loadLoopPackDirectory(packRoot, {
      loopgraphVersion: "0.2.0",
      hermesVersion: "1.0.0"
    });
    ...
    const evaluation = await runAppSyntheticConformance({
      ...
      compiled,
      installation,
      actor: "official-deal-risk-pack-test",
