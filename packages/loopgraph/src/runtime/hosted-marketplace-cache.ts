...
export async function ensureRemoteHostedMarketplaceArtifact(input: {
  client: HostedMarketplaceClient;
  projectRoot: string;
  appId: string;
  version?: string;
  versionRange?: string;
  ...
  const app = await input.client.getApp(input.appId, {
    includeDeprecated: input.includeDeprecated ?? true
  });
  if (!app) {
    await evictRemoteHostedSources(input);
    throw new Error(`Hosted marketplace app not found: ${input.appId}`);
  }
  await reconcileRemoteHostedSources(input.projectRoot, app);
  ...
  if (input.artifactDigest && input.artifactDigest !== version.digest) {
    throw new Error(
      "Hosted marketplace artifact digest does not match the reviewed request"
    );
  }

  const stateRoot = marketplaceStateRoot(input.projectRoot);
  ...
  const downloaded = await input.client.downloadArtifact({
    appId: app.id,
    version: version.version,
    artifactDigest: version.digest
  });
  if (
    downloaded.artifactDigest !== version.digest ||
    downloaded.publisherKey.publisherId !== app.publisher.id
  ) {
    throw new Error("Hosted marketplace download identity does not match catalog metadata");
  }
...
export async function hasRemoteHostedMarketplaceArtifact(input: {
  projectRoot: string;
  appId: string;
  version?: string;
  versionRange?: string;
  ...
  const versions = await localMarketplace(input.projectRoot)
    .listAppVersions(input.appId);
...
async function evictRemoteHostedSources(input: {
  projectRoot: string;
  appId: string;
  version?: string;
  versionRange?: string;
...
async function reconcileRemoteHostedSources(
  projectRoot: string,
...
function localMarketplace(projectRoot: string) {
  return new LocalAppMarketplace(marketplaceStateRoot(projectRoot));
}

function marketplaceStateRoot(projectRoot: string) {
  return path.join(projectRoot, ".loopgraph", "apps", "marketplace");
}
