...
11. Use \`loopgraph_app_duplicate\` to create a namespaced private derived app. Pass the exact current source artifact digest and source revision time, and preserve the private App ID plus overlay operations for exact recovery. Use \`loopgraph_app_detach\` only after the user understands that it pins a local immutable snapshot and permanently stops upstream updates.
