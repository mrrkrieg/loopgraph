...
6. Use \`loopgraph_app_install_status\`, \`loopgraph_app_onboarding_get\`, and \`loopgraph_app_diff\` as the source of truth for installed state, journey progress, configuration digest, overlay revision, active LoopSpecs, lifecycle receipts, recoverable lifecycle operations, revision history, and update availability. If an install, uninstall, activation, rollout, update, or rollback operation is \`prepared\` or \`requires_reconciliation\`, retry that exact content-bound request before starting another lifecycle action for the installation. An update retry must use the original reviewed plan, recorded permission approvals, source artifact, and same accountable actor; its prepared journal may finish after the original plan window expires. A rollback retry must use the recorded source artifact and the same accountable actor; never select a replacement revision during recovery.
