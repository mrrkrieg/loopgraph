...
6. Use \`loopgraph_app_install_status\`, \`loopgraph_app_onboarding_get\`, and \`loopgraph_app_diff\` as the source of truth for installed state, journey progress, configuration digest, overlay revision, active LoopSpecs, lifecycle receipts, recoverable lifecycle operations, revision history, and update availability. If an install, uninstall, activation, rollout, configure, overlay, repair, update, or rollback operation is \`prepared\` or \`requires_reconciliation\`, retry that exact content-bound request before starting another lifecycle action for the installation. A configure retry must use the original confirmed values, prior configuration digest, and same accountable actor; Loopgraph stores only bounded digests in its recovery journal. An overlay retry must use the original operations, prior overlay revision, source artifact, and same accountable actor; Loopgraph accepts only the recorded source or target owned LoopSpec topology and stores no operations in the journal. A repair retry must use the exact source artifact digest and source revision time returned by the recovery journey with the same accountable actor; Loopgraph accepts only the recorded pre-repair or regenerated topology. An update retry must use the original reviewed plan, recorded permission approvals, source artifact, and same accountable actor; its prepared journal may finish after the original plan window expires. A rollback retry must use the recorded source artifact and the same accountable actor; never select a replacement revision during recovery.
