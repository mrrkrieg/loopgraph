...
export const PROVIDER_ONBOARDING_CATALOG: ProviderOnboardingProfile[] = [
  ...
  detectorAdminProfile("bigquery", "Google BigQuery", "data_warehouse", "service_account", "https://cloud.google.com/bigquery/docs/authentication/service-account-file", ["management.company_metric_anomaly", "finance.forecast_variance_detected", "capacity.plan_changed"], "bigquery-detector/v1", 60),
  detectorAdminProfile("snowflake", "Snowflake", "data_warehouse", "connected_app", "https://docs.snowflake.com/en/developer-guide/sql-api/authenticating", ["management.company_metric_anomaly", "finance.forecast_variance_detected", "capacity.plan_changed"], "snowflake-detector/v1", 60)
];
...
function detectorAdminProfile(providerId: ProviderId, label: string, systemClass: ProviderOnboardingProfile["systemClass"], credentialKind: "api_key" | "service_account" | "connected_app", instructionsUrl: string, eventFamilies: string[], transformerId: string, pollCadenceMinutes: number) {
  return providerOnboardingProfileSchema.parse({ schemaVersion: "provider-onboarding/v1alpha1", providerId, label, systemClass, controlPlane: "hermes", authorization: { mode: "admin_managed", instructionsUrl, credentialKind }, ingestion: { mode: "scheduled_detector", eventFamilies, transformerId, signatureStrategy: "Hermes signed detector receipt", pollCadenceMinutes }, leastPrivilegeNotes: ["Use a dedicated read-only identity, store its credential only in the Connector Broker vault, and rotate it through Hermes."] });
}
