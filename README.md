...
Hosted App lifecycle state is also distributed rather than written to one deployment's filesystem. Organization, project, and server-derived workspace scope key a revisioned Supabase installation registry; a bounded lease serializes onboarding, install, configure, test, activation, update, rollback, pause, and uninstall mutations across instances. Browser lifecycle actions derive the accountable actor from the authenticated session. The App registry and its lock commit together, while generated LoopSpecs commit through the existing distributed LoopSpec registry. Draft save/reset actions append their bounded metadata-only audit event in that same registry transaction. Activation approval and one-time receipt consumption do the same with separate `app.activation.approved` and `app.activation.consumed` events; only content digests, states, expiry, and bounded counts enter the security chain, while approval text, evidence references, provider data, and raw App/install identities stay in the authoritative registry. Install, activation, pause, resume, rollback, and uninstall use a bounded, metadata-only recovery journal before crossing into LoopSpec, mapping, or context stores. Activation recovery binds the exact receipt and digest, artifact, source state, target mode, actor, and owned loop set; it can finish an already-started attempt after receipt expiry but cannot issue new authority, tolerate topology drift, or consume a receipt twice. Pause/resume recovery binds the exact source state, source revision timestamp, approved mode, target state and mode, actor, artifact, and owned loop set; it cannot silently switch rollout modes, adopt a changed topology, or mistake a later pause/resume cycle for a replay. Rollback recovery binds the complete source installation and ownership digests, prior revision, original workspace revision, actor, and exact source and target LoopSpec inventories before rematerialization. It accepts only the recorded source or already-applied target topology, finishes the App registry and receipt exactly once, and never selects a replacement revision during recovery. Uninstall recovery binds the complete source-installation and ownership digests, source workspace revision, exact pre-removal and permitted post-removal LoopSpec inventories, actor, and a digest of the accountable reason. It stores no raw reason in the recovery metadata, accepts only the original actor and reason on retry, and fails closed if the graph is neither exactly before nor exactly after the recorded removal. If a worker stops after one store commits, the same content-bound request resumes idempotently and App status, Hermes onboarding, and the browser expose the unfinished operation with its affected resource counts and exact safe retry. Competing mutations remain blocked until reconciliation completes. Protected operational metrics expose aggregate pending, interrupted, stale, affected-workspace, and oldest-age signals without placing App IDs or customer metadata in telemetry. A stale plan, conflicting operation, concurrent lease, expired lease, cross-workspace payload, oversized registry, or unavailable database fails closed. Local mode keeps the same service contract with an owner-private atomic file registry.
