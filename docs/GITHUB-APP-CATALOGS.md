...
- Indexed versions retain the exact source ID, Git URI, commit, catalog snapshot digest, trust policy, and synchronization time. A later refresh can remove only records owned by that source and cannot overwrite another source with a divergent digest for the same semantic version.
