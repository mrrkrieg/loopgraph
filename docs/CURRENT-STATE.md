...
- A versioned, read-only App onboarding journey now derives the same eight-step progress for Hermes, CLI, and browser from durable connector, mapping, configuration, install, test, and lifecycle state. It returns only unresolved questions and blockers plus one exact safe next action, and stops separately for installation and shadow-activation approval. Its content-bound impact review exposes exact additions, reused company resources, permissions, output metrics, evidence edges, and blocking shared-object contract conflicts instead of hiding a conflict behind a planning error. Non-live mode activation is enforced with a short-lived, content-bound, one-time approval receipt rather than an actor label or prompt-only confirmation.
