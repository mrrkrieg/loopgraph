...
- Settings now projects tenant-scoped detector health and five-run history, links emitted evidence to Hermes activity, and provides MFA-gated pause, resume, run-now, and exact-window retry controls. Accepted and denied controls are transactional audit-chain events; browser roles cannot execute the detector RPCs or read scheduler tables.
