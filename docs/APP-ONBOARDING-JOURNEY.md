...
- An unfinished install, activation, pause, resume, configure, overlay, repair, duplicate, update, rollback, or uninstall returns `recover_lifecycle`; every competing test, activation, configuration, overlay, repair, update, rollback, duplicate, detach, pause, resume, or removal mutation remains blocked until the exact operation completes.
