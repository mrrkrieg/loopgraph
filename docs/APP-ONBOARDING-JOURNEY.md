...
- An unfinished install, activation, pause, resume, configure, overlay, repair, duplicate, detach, update, rollback, or uninstall returns `recover_lifecycle`; every competing test, activation, configuration, overlay, repair, update, rollback, duplicate, detach, pause, resume, or removal mutation remains blocked until the exact operation completes.
- Duplicate recovery returns the exact source artifact and revision, private App ID, overlay digest, target installation, and target loop inventory. The original overlay values remain with the caller; the journal keeps only their digest. Retrying reconciles namespaced LoopSpecs, shared field mappings, approved company-context consumers, ownership, and the derived registry record exactly once.
- Detach recovery returns the exact source artifact and revision, source/target owned LoopSpec inventory, and confined immutable snapshot path. The snapshot is staged and artifact-verified before its atomic promotion; retry accepts only the same actor and exact installed state, tolerates unrelated company-graph revisions, rejects drift in App-owned loops, and returns the original lifecycle receipt after completion.
