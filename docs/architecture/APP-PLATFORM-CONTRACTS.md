...
Operational maturity is one shared runtime contract, exposed as `loopgraph_app_maturity_get` to Hermes, MCP, CLI, and the browser. Version `loopgraph-app-operational-maturity/v1alpha2` adds an ordered freshness result for replay, completed run, observed outcome, and observed value; it exposes each evidence status and expiry, the earliest `validUntil`, and a seven-day `renewalRecommendedAt`. The installed-App UI renders the same clock, so operators can renew proof before a downgrade rather than discovering expiry during activation. The service joins exact App evaluations, current installation readiness, App-owned completed Hermes runs, observed outcomes, observed value-ledger entries, and independent verification receipts. Local deployments persist verifier trust and receipts in a workspace-bound, atomically replaced `0600` registry. Adding trust requires an approver, approval reference, and timestamp; revocation requires a separate accountable actor, reference, and timestamp. Receipt import verifies the Ed25519 signature against an active key before writing and rechecks the exact installed `installationId`, `appId`, and `artifactDigest`. The registry stores only public keys and signed receipts; verifier private keys remain in the independent verifier's own HSM, KMS, or vault.

Fleet renewal uses the versioned `loopgraph-app-evidence-renewal-plan/v1alpha1` projection. The App service reads the tenant registry, App-owned operational evidence, and verifier registry once, then applies the same maturity evaluator to every installation. Full-fleet counts remain independent of status filters; `totalMatched` reveals filtering and at-most-100 truncation. Ranking is deterministic: invalid or expired evidence is critical, renew-soon evidence is high, incomplete evidence is medium, and current/not-yet-applicable Apps are non-urgent. Each item contains only installation/App identities, artifact digest, maturity, bounded evidence statuses, deadlines, and one safe next action. It contains no provider payload, object value, credential, vault reference, or arbitrary operation, and reading it cannot run a replay, create approval, promote an App, or execute provider work.

The local supervisor is a read-only consumer of that contract, not a second maturity evaluator. Its existing hourly App component parses the versioned response, reports updates and evidence status together, blocks only invalid proof, degrades for expired or renew-soon proof, and leaves incomplete-but-valid setup as healthy operational infrastructure with a visible maturity gap. It persists only bounded counts, installation/App identity for the first actionable item, its evidence deadline, and the service-authored safe next action in the owner-private supervisor status file. The supervisor cannot invoke that action; Hermes or an accountable operator must cross every existing replay, evidence, approval, and activation boundary separately.

Hosted operations consume the same contract through a tenant-derived server boundary. Protected Prometheus output contains aggregate evidence states, fleet size, returned-item count, and truncation only; it excludes installation/App identities, action text, provider values, credentials, and object data. An hourly workload with only `schedule.app_evidence_health` emits the same aggregate observation. Invalid, expired, or renew-soon proof degrades operations without removing otherwise healthy workers, while unavailable, malformed, cross-workspace, stale, or future-dated projections fail readiness closed. Neither the metrics request nor the schedule can invoke the plan's next action.
